...
function saveApiKey() {
  ...
  apiKey = val;
  try {
    localStorage.setItem("oi_analyzer_api_key", val);
  } catch (e) {}
...
function parseCSV(text) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (
      up.includes("STRIKE") &&
      (up.includes(",OI,") || up.includes("VOLUME"))
    ) {
      headerIdx = i;
      break;
    }
  ...
  const headers = parseCSVRow(lines[headerIdx]).map((h) =>
    h.toUpperCase().trim(),
  );
  let strikeCol = headers.indexOf("STRIKE");
  if (strikeCol === -1)
    strikeCol = headers.findIndex((h) => h.includes("STRIKE"));
  ...
  if (rows.length === 0) {
    alert(
      "Could not parse the CSV. Please make sure it is the Bank Nifty options chain CSV from NSE.",
    );
    return;
  }
  ...
  const maxCallRow = rows.reduce(
    (m, r) => (r.callOI > m.callOI ? r : m),
    rows[0],
  );
  ...
  const atmRow =
    liquid.length > 0
      ? liquid.reduce(
          (m, r) =>
            Math.abs(r.callLTP - r.putLTP) < Math.abs(m.callLTP - m.putLTP)
              ? r
              : m,
          liquid[0],
        )
...
function renderDashboard(data) {
  const { rows, totalCallOI, totalPutOI, pcr, maxCallRow, maxPutRow, atmRow } =
    data;
  ...
  // Metrics grid
  const pcrColor =
    biasClass === "bull" ? "bull" : biasClass === "bear" ? "bear" : "warn";
  document.getElementById("metricsGrid").innerHTML = [
    ...
    {
      label: "Max Call Strike",
      val: maxCallRow.strike.toLocaleString(),
      cls: "bear",
    },
    {
      label: "Max Put Strike",
      val: maxPutRow.strike.toLocaleString(),
      cls: "bull",
    },
  ...
  const maxOI =
    Math.max(...visible.map((r) => Math.max(r.callOI, r.putOI))) || 1;
  ...
  document.getElementById("levelList").innerHTML = [
    {
      name: "Strong Resistance (Max Call OI)",
      val: maxCallRow.strike.toLocaleString(),
      cls: "bear",
    },
    {
      name: "Resistance Zone",
      val: (maxCallRow.strike + 200).toLocaleString(),
      cls: "bear",
    },
    {
      name: "ATM (Current Price Area)",
      val: atmRow.strike.toLocaleString(),
      cls: "blue",
    },
    {
      name: "Support Zone",
      val: (maxPutRow.strike - 200).toLocaleString(),
      cls: "bull",
    },
    {
      name: "Strong Support (Max Put OI)",
      val: maxPutRow.strike.toLocaleString(),
      cls: "bull",
    },
  ]
...
async function generateAISignal(data) {
  ...
  const top10 = [...rows]
    ...
    .map(
      (r) =>
        `  Strike ${r.strike}: CallOI=${r.callOI.toLocaleString()}, PutOI=${r.putOI.toLocaleString()}, CallChg=${r.callChg > 0 ? "+" : ""}${r.callChg}, PutChg=${r.putChg > 0 ? "+" : ""}${r.putChg}`,
    )
    .join("\n");
...
function renderTradeplan(r) {
  ...
  document.getElementById("riskBox").innerHTML =
    `⚠ TRAP TO AVOID: ${escHtml(r.risk_warning)}`;
}
